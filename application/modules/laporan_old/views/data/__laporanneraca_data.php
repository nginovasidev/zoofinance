                        ...
                        <table border="1" width="100%" class="table table-bordered">
                            ...
                            <?php 
                                foreach ($query as $item) { 
                                    ...
                                    if ($item->is_saldo==0) {
                                            // AND b.tgltrx>'".date("Y", strtotime($_POST["periode"]))."-01-01' AND b.tgltrx<'".$_POST['periode']."' 
...
and b.tgltrx between '".date("Y", strtotime($_POST["periode"]))."-01-01' and '".date('Y-m-d', strtotime($_POST['periode']. ' - 1 days'))."' 
                                            and '0'=b.is_deleted
                                             group by a.kdakun) c on a.kdakun=c.kdakun
                                            left join (select a.kdakun, ifnull(sum(b.d),0) as debet , ifnull(sum(b.k),0) as kredit from m_akun a left join t_keuangan b on a.`kdakun`=b.kdakun where b.tgltrx between '".$_POST['periode']."' and '".$_POST['periode']."' 
                                            and '0'=b.is_deleted
