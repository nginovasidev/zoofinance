                        ...
                        <div class="tab-pane fade active show" id="tab-form" role="tabpanel" aria-labelledby="tab-form">
                            <form data-plugin="parsley" data-option="{}" id="form">
                                ...
                                <div class="form-group">
                                    <label for="id_polda">Polda</label>
                                    <select class="form-control sel2" id="id_polda" name="id_polda" required>
                                    </select>
                                </div>
                                <div class="form-group">
                                ...
                                <div class="form-group">
                                    <label for="nama_polrestabes">Nama Polres</label>
                                    <input type="text" class="form-control" id="nama_polrestabes" name="nama_polrestabes" required autocomplete="off" placeholder="Tentukan nama polres">
                                </div>
                                <div class="form-group">
                                    <label for="singkat_polrestabes">Singkatan Polres</label>
                                    <input type="text" class="form-control" id="singkat_polrestabes" name="singkat_polrestabes" required autocomplete="off" placeholder="Tentukan singkatan polres">
                                </div>
                            ...
                            <div class="table-responsive">
                                <table id="datatable" class="table table-theme table-row v-middle">
                                    <thead>
                                        <tr>
                                            <th><span>#</span></th>
                                            <th><span>Polda</span></th>
                                            <th><span>Polres</span></th>
                ...
                if (result.value) {
                    ...
                    $.ajax({
                        ...
                        success: function(result) {
                            Swal.close();
                            if (result.success) {
                                Swal.fire('Sukses', 'Berhasil menyimpan polres', 'success');
                                $('#form').trigger("reset");
                                table.ajax.reload();
                            } else {
            ...
            $.ajax({
                ...
                success: function(result) {
                    ...
                    if (result.success) {
                        ...
                        $('#id_polda').select2("trigger", "select", {
                            data: {
                                id: result.data.id_polda,
                                text: result.data.nama_polda
                            }
                        });

                        $('#id_kota').select2("trigger", "select", {
                            data: {
                                id: result.data.id_kota,
                                text: result.data.nama_polrestabes
                            }
                        });

                        setTimeout(function() {
                            $('#id_polda').val(result.data.id_polda).trigger('change');
                ...
                if (result.value) {
                    $.ajax({
                        ...
                        success: function(result) {
                            Swal.close();
                            if (result.success) {
                                Swal.fire('Sukses', 'Berhasil menghapus polres', 'success');
                                table.ajax.reload();
                            } else {
                                Swal.fire('Error', result.message, 'error');
                            }
