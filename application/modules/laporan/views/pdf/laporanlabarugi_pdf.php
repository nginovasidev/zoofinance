...
.tabela table tr:nth-child(even) {
    background:#F0F8FF;
                    ...
                    <div>
                        ...
                        <div align="center">TANGGAL <?=date("d M Y" , strtotime($_GET['periode_awal']))?> - <?=date("d M Y" , strtotime($_GET['periode']))?></div>
                        <br>
                        <div class="tabela">
                        <table border="1" width="100%" class="table table-bordered">
                            ...
                            <?php 
                                foreach ($query as $item) { 
                                    ...
                                    //         from m_akun a
                                    //         left join t_keuangan b on a.`kdakun`=b.kdakun
                                    //         left join (select a.kdakun, if(a.normalbalance='D',if('2021'='".date("Y", strtotime($_GET["periode"]))."', a.saldoawal_d , 0)+ifnull(sum(b.d),0)-ifnull(sum(b.k),0),if('2021'='".date("Y", strtotime($_POST["periode"]))."', a.saldoawal_k , 0) +ifnull((sum(b.k)-sum(b.d)),0)) as saldoawal from m_akun a left join t_keuangan b on a.`kdakun`=b.kdakun 
                                    //         and b.tgltrx between '".date("Y", strtotime($_GET["periode"]))."-01-01' and '".$tglawal."' 
                                    //         and '0'=b.is_deleted
                                    //          group by a.kdakun) c on a.kdakun=c.kdakun
                                    ...
                                    ?>
                                <tr>
                                    <td <?=$style?> <?=$is_bold?> ><!-- <?=$pr?> --><?=$this->uri->segment('2')=='excel'? "'": ""?><?=$item->kdakun?></td>
