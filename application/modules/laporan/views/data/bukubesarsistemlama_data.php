            ...
            <div class="card">
                ...
                <div class="card-body">
                    <div>
                        ...
                        $querylegerdetail = $this->db->query("SELECT a.namaakun , a.normalbalance , JSON_ARRAYAGG(JSON_OBJECT('tgl_trx', b.tgtran , 'ket' , b.ket , 'd' , b.d , 'k' , b.k , 'saldo' , NULL) ) AS arr_detailakun, c.saldo from mast_akun a join keuangan_trans b on a.`kdakun`=b.kdakun left join (select a.kdakun, if(a.normalbalance='D',a.saldoawal_d+ifnull(sum(b.d),0)-ifnull(sum(b.k),0),a.saldoawal_k+ifnull((sum(b.k)-sum(b.d)),0)) as saldo from mast_akun a  left join keuangan_trans b on a.`kdakun`=b.kdakun and b.tgtran between '2021-01-01' and '".$tglawal."' group by a.kdakun) c on a.kdakun=c.kdakun  where b.tgtran between '".$_POST['periode_awal']."' and '".$_POST['periode_akhir']."' $wherebyakun group by a.kdakun");
